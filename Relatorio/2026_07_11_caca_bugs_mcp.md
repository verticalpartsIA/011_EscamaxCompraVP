...
## 9. ADENDO (11/07/2026) — Correções aplicadas ✅

> Contexto: o portal ainda não está em produção para usuários — os pedidos de fev/mar são
> dados de teste/mock. Portanto o item §1 (POs órfãos em Piçarras) não exige reconciliação
> financeira real. Os bugs vivos de código foram todos corrigidos nesta branch:

| Item | Correção |
|------|----------|
| §3 Divergência silenciosa de itens | `omieClient.js` agora **aborta o pedido inteiro** (compra e venda) se qualquer item não puder ser incluído (não encontrado/não clonável/sem preço), listando os itens com problema na mensagem de erro — nada de `continue` silencioso. Na venda, o abort dispara a compensação já existente (cancela a compra criada). |
| §4 Estoque negativo vendável | Novo `server/services/estoqueService.js` valida o saldo na tabela `estoque_vp` (fonte correta, sync horária da Omie) em 3 pontos: preflight (`GET/POST /api/checkout/preflight`), checkout (`POST /api/checkout/processar`) e na aprovação final (`pedidoOmieService.criarPedidosOmie`), pois o saldo pode mudar entre checkout e aprovação. Política: **bloqueia** pedido com quantidade > disponível (soma códigos repetidos no carrinho). Códigos fora dos prefixos rastreados (VPEL/VPER/VPB) passam com warning. |
| §5 Sem teto de quantidade | `validarItens` (checkoutPreflight.js) rejeita quantidade > 1.000 por item (ajustável via `CHECKOUT_QTD_MAX_ITEM` no `.env`). |
| §6 Catálogo com `estoque_atual` furado | Frontend (`ProdutosVPPage`, `PecasSemEstoquePage`, `OutrosFornecedoresPage`) agora lê o saldo de `estoque_vp.estoque_disponivel` via helper compartilhado `client/src/lib/estoqueVP.js`. O catálogo também bloqueia o botão "Adicionar" para item sem saldo, e o split de Outros Fornecedores usa o saldo real. `omie_produtos.estoque_atual` não é mais usado em lugar nenhum da UI. |
| §7 Sem rate-limit de login | `authController.js` bloqueia por 15 min após 5 falhas seguidas do mesmo e-mail (ajustável via `LOGIN_MAX_FALHAS` / `LOGIN_JANELA_MINUTOS`), com log de auditoria `login.bloqueado`. |

Validações executadas contra o Supabase real: VPEL-151 (disponível -28) bloqueado; VPB-001
(disponível 7) passa com 5 e bloqueia com 10; soma de código repetido no carrinho respeitada;
código não rastreado passa com warning; teto de quantidade rejeita o caso real de 28.300 un.
Build do frontend (Vite) OK.
