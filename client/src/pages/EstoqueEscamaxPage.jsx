...
export default function EstoqueEscamaxPage() {
    ...
    const load = useCallback(async () => {
        ...
        setError(null);
        try {
            const data = await fetchEstoqueEscamax(filial.id);
            setProdutos(data);
            if (data.length > 0) {
                ...
                {loading ? (
                    ...
                    <div className="flex flex-col items-center gap-2 py-16 text-center">
                        ...
                        <p className="text-sm font-medium text-neutral-500">
                            {search ? 'Nenhum produto encontrado' : 'Nenhum dado sincronizado ainda para esta filial'}
                        </p>
                    </div>
                ) : (
